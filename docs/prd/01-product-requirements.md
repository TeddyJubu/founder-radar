...
| **FR-8.2** | Ten commands: `/today`, `/run`, `/run <fund>`, `/fund <name>`, `/why <company>`, `/status`, `/week`, `/sheet`, `/fix`, `/help`. |
...
- Server is a managed Linux VPS: 1 vCPU, 4 GB RAM, ~50 GB disk, Ubuntu.
...
- The configured Google service account and spreadsheet remain in use.
...
|---|---|---|---|
| Companies House volume is too noisy even after filtering | Medium | High | Qualification signals (SH01, grant, spinout, press) gate Track B separately. A live website and a prior appointment are proven but not admitting by default. Track A works regardless. Threshold sweep tool tunes it against Aryan's labels. |
...
Every request Aryan made in the project brief, and where it is met.
