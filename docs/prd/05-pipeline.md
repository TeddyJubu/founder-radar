...
**Rate-limit budget** is counted in **requests, not companies** — full enrichment is 4–8 calls per company, not 2. `settings.max_enrichment_requests_per_run` (default 500) is decremented by the limiter on every call. Roughly one third is reserved for each of filing history, officers/PSC, and prior appointments so a growing queue cannot starve the evidence that proves a repeat founder. When it runs out, incomplete rows stay queued with `enriched_at IS NULL` or a pending appointment marker. See `04-sources.md` §3.4a for the pass ordering.
