...
Target machine: **Hostinger KVM 1** — 1 vCPU, 4 GB RAM, ~50 GB disk, Ubuntu 24.04.

---

## 1. Before anything else — rotate the Google key

> ⚠️ **The Google service-account private key was shared in the Fiverr chat thread.** Any credential that has travelled through a chat should be treated as exposed.

1. Google Cloud Console → IAM & Admin → Service Accounts → `founder-finder@founder-finder-502208.iam.gserviceaccount.com`
...
The spreadsheet ID (`154BCy0g...`) is not a secret — it is only a document identifier. The private key is.
...
RADAR_SHEET_ID=154BCy0g3r8JEDhFk5fmy9OWKszNRjFVRYm9Gqb9SOQA
TELEGRAM_BOT_TOKEN=...
TELEGRAM_CHAT_ID=...
...
| Item | Monthly | Notes |
|---|---|---|
| Hostinger KVM 1 | ~£6 | already purchased |
