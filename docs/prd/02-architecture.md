...
|---|---|---|---|
| Matches the agreed offer wording | ✅ | ❌ | ✅ |
...
> **Action required before build:** the Google service-account private key was shared in a third-party chat. Any key that has travelled through a chat thread should be treated as exposed. Generate a fresh key in the Google Cloud console, delete the old one, and share only the new file's path — never its contents.
