...
def test_the_ledger_never_renders_an_unknown_as_a_failure(db):
    ...
    The full-model score keeps the unknown in its denominator, but it is still
    "nobody could find out", not "it scored zero"; a digest that marks them
    alike undoes that in the one place the client actually reads each morning."""
