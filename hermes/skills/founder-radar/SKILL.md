...
is gated the same way and will refuse on hash drift or missing Hermes when
there are reviewable scores. `--force` alone does **not** bypass the gate —
ops must also set `RADAR_ALLOW_FORCE_PUBLISH=1` (never in systemd).
