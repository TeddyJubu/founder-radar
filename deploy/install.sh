...
if [ -f "$HERE/hermes-wrap.sh" ]; then
  chmod 755 "$HERE/hermes-wrap.sh"
fi
...
# Never ship a unit that sends digests without the publish gate. Old bak
# files with `digest --today --send` must not be reinstalled by hand either.
unit_file="$UNIT_DIR/founder-radar.service"
if ! grep -qE '^ExecStart=.*/founder-radar[[:space:]]+publish[[:space:]]+--send' "$unit_file"; then
  say "FATAL: $unit_file must ExecStart \`publish --send\` (Hermes-gated path)"
  exit 1
fi
if grep -qE '^ExecStart=.*/founder-radar[[:space:]]+digest[[:space:]].*--send' "$unit_file"; then
  say "FATAL: $unit_file still has raw \`digest --send\` — refuse to enable"
  exit 1
fi
# Bak copies of the ungated unit are a footgun next to install.sh.
rm -f "$HERE"/founder-radar.service.bak* "$UNIT_DIR"/founder-radar.service.bak*

# Always re-assert Hermes ACLs before run/publish (Hermes chmod clears mask).
# Leading `+` runs as root — after mask::--- only root/owner can setfacl.
install -d "$UNIT_DIR/founder-radar.service.d"
cat > "$UNIT_DIR/founder-radar.service.d/hermes-acl.conf" <<EOF
[Service]
# Re-assert ACLs before run/publish — Hermes chmod can clear ~/.hermes mask.
ExecStartPre=+$APP_DIR/deploy/hermes-acl.sh
EOF
chmod 644 "$UNIT_DIR/founder-radar.service.d/hermes-acl.conf"

# Allow the radar user to refresh ACLs mid-publish (hermes-wrap / Python).
if command -v visudo >/dev/null 2>&1 || [ -d /etc/sudoers.d ]; then
  cat > /etc/sudoers.d/founder-radar-hermes-acl <<EOF
# Generated by deploy/install.sh — radar may restore Hermes ACLs only.
${APP_USER} ALL=(root) NOPASSWD: $APP_DIR/deploy/hermes-acl.sh
EOF
  chmod 440 /etc/sudoers.d/founder-radar-hermes-acl
fi
...
write_hermes_service_dropin() {
  local drop_dir="$UNIT_DIR/founder-radar.service.d"
  local drop_in="$drop_dir/hermes.conf"
  if [ -z "$HERMES_HOME" ] || [ ! -d "$HERMES_HOME/.hermes" ]; then
    rm -f "$drop_in"
    return 0
  fi
  install -d "$drop_dir"
  {
    printf '# Generated by deploy/install.sh — do not edit by hand.\n'
    printf '# Hermes Today QA writes under the operator home (config, logs,\n'
    printf '# ~/.local/share). ProtectHome=read-only keeps other homes locked.\n'
    printf '[Service]\n'
    printf 'ReadWritePaths=%s\n' "$HERMES_HOME"
  } > "$drop_in"
  chmod 644 "$drop_in"
  say "Today QA Hermes path: $HERMES_HOME"
}

# setfacl is required so mode-700 homes are usable by $APP_USER.
ensure_acl_tool() {
  if command -v setfacl >/dev/null 2>&1; then
    return 0
  fi
  say "installing acl (setfacl) for Today QA Hermes access"
  apt-get install -y -qq acl
}

grant_radar_hermes_access() {
  local home="$1"
  ensure_acl_tool || {
    say "setfacl unavailable — Today QA may not read $home/.hermes"
    return 0
  }
  [ -d "$home" ] || return 0
  # Hermes also writes ~/logs and ~/.local/share outside ~/.hermes.
  setfacl -R -m "u:${APP_USER}:rwX" "$home" 2>/dev/null || true
  setfacl -d -m "u:${APP_USER}:rwX" "$home" 2>/dev/null || true
  # Canonical refresh (mask + logs + binary) — same script as ExecStartPre.
  if [ -x "$APP_DIR/deploy/hermes-acl.sh" ]; then
    HERMES_USER="${HERMES_USER:-}" APP_USER="$APP_USER" \
      "$APP_DIR/deploy/hermes-acl.sh" || true
  fi
}

if [ -n "$HERMES_HOME" ] && [ -d "$HERMES_HOME/.hermes" ]; then
  install_skill "$HERMES_HOME" "$HERMES_USER"
  grant_radar_hermes_access "$HERMES_HOME"
  write_hermes_service_dropin
  systemctl daemon-reload
else
  ...
  rm -f "$UNIT_DIR/founder-radar.service.d/hermes.conf"
  systemctl daemon-reload
fi
...
write_hermes_env() {
  local real_bin="${HERMES_BIN:-}"
  local wrap="$APP_DIR/deploy/hermes-wrap.sh"
  # Re-install must not nest the wrap as HERMES_REAL_BIN.
  case "$real_bin" in
    *hermes-wrap.sh)
      real_bin="${HERMES_REAL_BIN:-}"
      if [ -z "$real_bin" ] && [ -n "${HERMES_HOME:-}" ] \
          && [ -x "$HERMES_HOME/.local/bin/hermes" ]; then
        real_bin="$HERMES_HOME/.local/bin/hermes"
      fi
      ;;
  esac
  local bin_out="$real_bin"
  if [ -x "$wrap" ] && [ -n "$real_bin" ] && [ "$real_bin" != "$wrap" ]; then
    bin_out="$wrap"
  fi
  {
    printf 'HERMES_USER=%s\n' "${HERMES_USER:-}"
    printf 'HERMES_HOME=%s\n' "${HERMES_HOME:-}"
    printf 'HERMES_REAL_BIN=%s\n' "${real_bin}"
    printf 'HERMES_BIN=%s\n' "${bin_out}"
