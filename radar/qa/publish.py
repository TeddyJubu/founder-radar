    ...
    *,
    ...
    if use_hermes:
        try:
            ...
            reviewable = int(diagnosis.get("reviewable") or 0)
            allow_rules = (os.environ.get("RADAR_ALLOW_RULES_ONLY_PUBLISH") or "").strip().lower() in {
                "1", "true", "yes",
            }
            # Quiet zero-reviewable day may publish without Hermes. Any
            # shortlist/watchlist generation requires Hermes to have run.
            if reviewable > 0 and not allow_rules:
                report.ok = False
                report.warnings.append(
                    "refusing publish: Hermes unavailable while reviewable "
                    "scores exist (set RADAR_ALLOW_RULES_ONLY_PUBLISH=1 to override)"
                )
            else:
